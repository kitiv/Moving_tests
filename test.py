...
for i in d_name: # Цикл по датчикам
    Incl2=Incl1[[j for j, ltr in enumerate(NofS1) if ltr == i],:] #Выделение строк относящ к опред датчику из общего массива
    [p1, p1y, p2, p2y, p3, p3y, p4, p4y, p5, p5y, phase_time, step_time] = Phase_detection(Incl2) # Функция расчета фаз шага
    ...
    GP1=L/Nstep/H
    GP2=L/Nstep/Twalk
    GP3_1=0 # надо вытащить
    GP3_2=np.mean(p3y)
    [GP4,GP5,GP6,GP7,GP8,GP9,GP10,GP11]=[np.mean(phase_time[0,:]),np.mean(phase_time[1,:]),np.mean(phase_time[2,:]),
                                         np.mean(phase_time[3,:]),np.std(phase_time[0,:]),np.std(phase_time[1,:]),
                                         np.std(phase_time[2,:]),np.std(phase_time[3,:])]
    GP12_1=0 # встроить по точкам определение N_FOG
    GP12_2 = 0 # встроить по точкам определение T_средн_FOG

    Out_mass = {'Growth':H,'Sex': S, 'Age': A , 'Weight': W, 'GP1':GP1, 'GP2':GP2, 'GP3.1':GP3_1, 'GP3.2':GP3_2, 'GP4':GP4
                , 'GP5':GP5, 'GP6':GP6, 'GP7':GP7, 'GP8':GP8, 'GP1':GP9, 'GP1':GP9, 'GP1':GP10, 'GP11':GP11, 'GP12.1':GP12_1
                , 'GP12.2':GP12_2 } # Массив выходных параметров
